...
/**
 * \file        class/pagamentoarretrato.class.php
 * \ingroup     lezioni
 * \brief       This file is a CRUD class file for PagamentoArretrato (Create/Read/Update/Delete)
 */
...
/**
 * Class for PagamentoArretrato
 */
class PagamentoArretrato extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'pagamentoarretrato';
	 ...
	 */
	public $table_element = 'lezioni_pagamentoarretrato';

	/**
	 * @var string 		If permission must be checked with hasRight('lezioni', 'read') and not hasright('lezioni', 'pagamentoarretrato', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'lezioni';

	/**
	 * @var string 		String with name of icon for pagamentoarretrato. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'pagamentoarretrato@lezioni' if picto is file 'img/object_pagamentoarretrato.png'.
	 */
	...
	public $fields = array(
		...
		"status" => array("type" => "integer", "label" => "Status", "enabled" => "0", 'position' => 2000, 'notnull' => 1, "visible" => "1", "default" => "1", "index" => "1", "arrayofkeyval" => array("0" => "Bozza", "1" => "Convalidato", "9" => "Annullato"), "validate" => "1",),
	...
	//  */
	// public $table_element_line = 'lezioni_pagamentoarretratoline';
	...
	//  */
	// public $fk_element = 'fk_pagamentoarretrato';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'PagamentoArretratoline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'PagamentoArretrato', 'fk_element'=>'fk_pagamentoarretrato'));
	...
	//  */
	// protected $childtablesoncascade = array('lezioni_pagamentoarretratodet');

	// /**
	//  * @var PagamentoArretratoLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('lezioni', 'pagamentoarretrato', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('lezioni', 'pagamentoarretrato', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('lezioni', 'pagamentoarretrato_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'pagamentoarretrato/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'pagamentoarretrato/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'pagamentoarretrato/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'pagamentoarretrato/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->lezioni->dir_output.'/pagamentoarretrato/'.$oldref;
				$dirdest = $conf->lezioni->dir_output.'/pagamentoarretrato/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->lezioni->dir_output.'/pagamentoarretrato/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowPagamentoArretrato")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("PagamentoArretrato").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$baseurl = dol_buildpath('/lezioni/pagamentoarretrato_card.php', 1);
		$query = ['id' => $this->id];
		if ($option !== 'nolink') {
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowPagamentoArretrato");
	...
	public function getLinesArray()
	{
		...
		$objectline = new PagamentoArretratoLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_pagamentoarretrato:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('LEZIONI_MYOBJECT_ADDON')) {
			$conf->global->LEZIONI_MYOBJECT_ADDON = 'mod_pagamentoarretrato_standard';
		}
		...
		if (getDolGlobalString('LEZIONI_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefPagamentoArretrato $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			if (!empty($this->model_pdf)) {
				...
				$modele = getDolGlobalString('MYOBJECT_ADDON_PDF', 'standard_pagamentoarretrato');
			}
		}
...
/**
 * Class PagamentoArretratoLine. You can also remove this and generate a CRUD class for lines objects.
 */
class PagamentoArretratoLine extends CommonObjectLine
{
	// To complete with content of an object PagamentoArretratoLine
	// We should have a field rowid, fk_pagamentoarretrato and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'pagamentoarretrato'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_pagamentoarretrato'
