...
/**
 *  \file       lezione_document.php
 *  \ingroup    lezioni
 *  \brief      Tab for documents linked to Lezione
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->lezioni->multidir_output[$object->entity ? $object->entity : $conf->entity]."/lezione/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('lezioni', 'lezione', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Lezione")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Lezione"), -1, $object->picto);
...
$modulepart = 'lezioni';
$param = '&id='.$object->id;
//$relativepathwithnofile='lezione/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'lezione/'.dol_sanitizeFileName($object->ref).'/';
