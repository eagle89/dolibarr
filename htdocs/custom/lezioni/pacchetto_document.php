...
/**
 *  \file       pacchetto_document.php
 *  \ingroup    lezioni
 *  \brief      Tab for documents linked to Pacchetto
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->lezioni->multidir_output[$object->entity ? $object->entity : $conf->entity]."/pacchetto/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('lezioni', 'pacchetto', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Pacchetto")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Pacchetto"), -1, $object->picto);
...
$modulepart = 'lezioni';
$param = '&id='.$object->id;
//$relativepathwithnofile='pacchetto/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'pacchetto/'.dol_sanitizeFileName($object->ref).'/';
