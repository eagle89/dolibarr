...
/**
 *   	\file       pacchetto_list.php
 *		\ingroup    lezioni
 *		\brief      List page for pacchetto
 */
...
require_once __DIR__.'/class/pacchetto.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('lezioni', 'pacchetto', 'write');
	$permissiontodelete = $user->hasRight('lezioni', 'pacchetto', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Pacchetto';
	$objectlabel = 'Pacchetto';
...
$title = $langs->trans("Pacchetti");
//$help_url = "EN:Module_Pacchetto|FR:Module_Pacchetto_FR|ES:Módulo_Pacchetto";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/lezioni/pacchetto_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/lezioni/pacchetto_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
$topicmail = "SendPacchettoRef";
$modelmail = "pacchetto";
$objecttmp = new Pacchetto($db);
