...
/**
 *  \file       pagamentoarretrato_document.php
 *  \ingroup    lezioni
 *  \brief      Tab for documents linked to PagamentoArretrato
 */
...
$upload_dir = null;
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->lezioni->multidir_output[$object->entity ? $object->entity : $conf->entity]."/pagamentoarretrato/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('lezioni', 'pagamentoarretrato', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("PagamentoArretrato")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("PagamentoArretrato"), -1, $object->picto);
...
$modulepart = 'lezioni';
$param = '&id='.$object->id;
//$relativepathwithnofile='pagamentoarretrato/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'pagamentoarretrato/'.dol_sanitizeFileName($object->ref).'/';
