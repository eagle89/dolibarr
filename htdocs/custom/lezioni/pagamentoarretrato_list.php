...
/**
 *   	\file       pagamentoarretrato_list.php
 *		\ingroup    lezioni
 *		\brief      List page for pagamentoarretrato
 */
...
include_once __DIR__.'/class/pagamentoarretrato.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('lezioni', 'pagamentoarretrato', 'write');
	$permissiontodelete = $user->hasRight('lezioni', 'pagamentoarretrato', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'PagamentoArretrato';
	$objectlabel = 'PagamentoArretrato';
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/lezioni/pagamentoarretrato_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-lezioni page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/lezioni/pagamentoarretrato_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendPagamentoArretratoRef";
$modelmail = "pagamentoarretrato";
$objecttmp = new PagamentoArretrato($db);
